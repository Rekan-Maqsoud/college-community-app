...
const collectionIds = {
  posts: process.env.EXPO_PUBLIC_APPWRITE_POSTS_COLLECTION_ID,
  replies: process.env.EXPO_PUBLIC_APPWRITE_REPLIES_COLLECTION_ID,
};
...
const run = async () => {
  ...
  await runKind('posts', collectionIds.posts);
  await runKind('replies', collectionIds.replies);
};

run().catch((error) => {
  console.error('fixInteractiveAcl failed:', error?.message || error);
  process.exit(1);
});
